...
static char *test_board_read_from_fewer_edges_than_declared() {
	...
	bool read = board_read_from(&b, file);
	mu_assert("board_read_from devrait échouer si moins d'arêtes sont fournies "
			  "que déclarées",
			  read == false);
	board_destroy(&b);
	fclose(file);
...
static char *test_board_read_from_more_edges_than_declared() {
	...
	bool read = board_read_from(&b, file);
	mu_assert("board_read_from devrait échouer si plus d'arêtes sont fournies "
			  "que déclarées",
			  read == false);
	board_destroy(&b);
	fclose(file);
...
static char *test_board_read_from_edge_with_invalid_vertex() {
	...
	bool read = board_read_from(&b, file);
	mu_assert("board_read_from devrait échouer si une arête fait référence à "
			  "un sommet inexistant",
			  read == false);
	board_destroy(&b);
	fclose(file);
	return NULL;
...
static char *test_board_read_from_invalid_robbers() {
	...
	// Test avec valeur nulle pour Robbers
	char data[] = "Cops: 1\nRobbers: 0\nMax turn: 10\n"
				  "Vertices: 2\n0 0\n0 0\nEdges: 1\n0 1\n";
	FILE *file = tmpfile();
	fputs(data, file);
	...
	bool read = board_read_from(&b, file);
	mu_assert("ERREUR: Doit échouer avec Robbers=0", read == false);

	board_destroy(&b);
	return NULL;
}

static char *test_board_read_from_valid_autoedge() {
	board b;
	board_create(&b);

	// Test avec nombre d'arêtes incohérent
	char data[] =
		"Cops: 1\nRobbers: 1\nMax turn: 10\n"
		"Vertices: 3\n0 0\n0 0\n0 0\n"
		"Edges: 2\n0 1\n1 2\n2 0\n"; // Déclare 2 arêtes mais en fournit 3
	FILE *file = tmpfile();
	fputs(data, file);
	rewind(file);

	bool read = board_read_from(&b, file);
	mu_assert("ERREUR: Doit détecter le surplus d'arêtes", read == false);
...
char *(*tests_functions[])() = {test_board_Floyd_Warshall_chain,
								...
								test_board_read_from_valid_autoedge};
